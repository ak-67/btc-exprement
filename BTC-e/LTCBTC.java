...
public class LTCBTC {
	...
	public static void log(Object text) throws Exception{
		if(logger == null)logger = new FileWriter("ltc-btc.txt", true);
	...
	public static void main(String[] args) throws Exception{
		...
		while(true){
			...
			double ltc = json.getDouble("ltc");

			JSONObject btc_usd_depth = getPublicAPI("https://btc-e.com/api/2/btc_usd/depth");
			JSONObject ltc_usd_depth = getPublicAPI("https://btc-e.com/api/2/ltc_usd/depth");
			JSONObject ltc_btc_depth = getPublicAPI("https://btc-e.com/api/2/ltc_btc/depth");

			double btc_usd_ltc_btc = 1 * btc_usd_depth.getJSONArray("bids").getJSONArray(0).getDouble(0) * (1 - btc_usd_fee);
			btc_usd_ltc_btc = btc_usd_ltc_btc / ltc_usd_depth.getJSONArray("asks").getJSONArray(0).getDouble(0) * (1 - ltc_usd_fee);
			btc_usd_ltc_btc = btc_usd_ltc_btc * ltc_btc_depth.getJSONArray("bids").getJSONArray(0).getDouble(0) * (1 - ltc_btc_fee);

			double btc_ltc_usd_btc = 1 / ltc_btc_depth.getJSONArray("asks").getJSONArray(0).getDouble(0) * (1 - ltc_btc_fee);
			btc_ltc_usd_btc = btc_ltc_usd_btc * ltc_usd_depth.getJSONArray("bids").getJSONArray(0).getDouble(0) * (1 - ltc_usd_fee);
			btc_ltc_usd_btc = btc_ltc_usd_btc / btc_usd_depth.getJSONArray("asks").getJSONArray(0).getDouble(0) * (1 - btc_usd_fee);

			System.out.println(btc_usd_ltc_btc);log(btc_usd_ltc_btc);
			System.out.println(btc_ltc_usd_btc);log(btc_ltc_usd_btc);
