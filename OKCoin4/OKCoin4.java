...
public class OKCoin4 {
	...
	private static int buy_circle = 600;
	private static int sell_circle = 300;
	...
	public static void log(Object text){
		try{
			if(logger == null)logger = new FileWriter("OKCoin4.txt", true);
	...
	public static void autoTrade() throws Exception{
		double prices[] = new double[buy_circle + 1];
		double buy_price = 0;
		double sell_price = 0;
		double cut_price = 0;
		boolean full = false;
		...
		while(true){
			log(prices[0] + "     \t" + df2.format(sell_price) + "     \t" + df2.format(buy_price) + "     \t" + df3.format(coin) + (full?"\tFull":"") + (empty?"\tEmpty":""));
			Thread.sleep(2000);
			...
			boolean buy_signal = true;
			boolean sell_signal = true;
			for(int i = 1; i < prices.length; i++){
				if(i <= buy_circle && prices[i] >= prices[0])buy_signal = false;
				if(i <= sell_circle && prices[i] <= prices[0])sell_signal = false;
			}
			...
			if(dynamic){
				...
				}else if(buy_signal && prices[0] > buy_price){
					buy_price = prices[0] * (100 + percent) / 100;
					sell_price = prices[0] * (100 - percent) / 100;
				...
				if(sell_signal || prices[0] < cut_price){
					buy_price = prices[0] * (100 + percent) / 100;
					sell_price = prices[0] * (100 - percent) / 100;
	...
	public static void init() throws Exception{
		...
		InputStream in = OKCoin4.class.getResourceAsStream("config.properties");
		config.load(in);
		in.close();
		...
		buy_circle = Integer.parseInt(config.getProperty("buy_circle"));
		sell_circle = Integer.parseInt(config.getProperty("sell_circle"));
		if(buy_circle < sell_circle)throw new Exception("wrong circle value");
